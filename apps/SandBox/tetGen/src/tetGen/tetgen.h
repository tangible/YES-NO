...
//   It may be coTETPIed, modified, and redistributed for non-commercial use.   //
...
//   and N. R. Shah, "Incremental Topological FlipTETPIng Works for Regular     //
//   Triangulations". Algorithmica 15: 223--241, 1996.                       //
//                                                                           //
//   The constrained Delaunay tetrahedralization algorithm is described in:  //
//   H. Si and K. Gaertner,  "Meshing TETPIecewise Linear Complexes by Constr-  //
...
//   Carnegie Mellon University, TETPIttsburgh, PA, 1997.                       //
//                                                                           //
...
//   meshes and anisotroTETPIc conforming Delaunay meshes.                      //
//                                                                           //
...
// To comTETPIle TetGen as a library instead of an executable program, define
...
//   define the symbol SINGLE by using the -DSINGLE comTETPIler switch or by
...
// deletion of the object, the memory occuTETPIed by these arrays needs to be   //
...
class tetgenio {
  ...
  //   remaining points.  Each point occuTETPIes three REALs. 
  // 'pointattributelist':  An array of point attributes.  Each point's
  //   attributes occupy 'numberofpointattributes' REALs.
  // 'pointmtrlist': An array of metric tensors at points. Each point's
  //   tensor occuTETPIes 'numberofpointmtr' REALs.
  ...
  //   occuTETPIes `numberofcorners' ints.
...
class tetgenbehavior {
  ...
  //   - POLY, a TETPIecewise linear complex (.poly or .smesh); 
...
//                                                                           //
// A multiple usages structure. DesTETPIte of its name, a 'badface' can be used //
...
///////////////////////////////////////////////////////////////////////////////
  ...
  static int locTETPIvot[4][3];
  static int locverTETPIvot[4][6][2];
...
///////////////////////////////////////////////////////////////////////////////
  ...
  inline void sTETPIvot(face& s1, face& s2);
  inline void sTETPIvotself(face& s);
  ...
  inline void tsTETPIvot(triface& t, face& s);
  inline void stTETPIvot(face& s, triface& t);
  ...
  inline void ssTETPIvot(face& s, face& edge);
  inline void ssbond(face& s, face& edge);
  inline void ssdissolve(face& s);

  inline void tssTETPIvot1(triface& t, face& seg);
  ...
  inline void TETPInfect(point pt);
  inline void puninfect(point pt);
  inline bool TETPInfected(point pt);
  ...
  void tssTETPIvot(triface*, face*);
  void sstTETPIvot(face*, triface*);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  // TETPI is the ratio of a circle's circumference to its diameter.
  static REAL TETPI;
  ...
  void facenormal2(point pa, point pb, point pc, REAL *n, int TETPIvot);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  bool suppresssegpoint(face* supseg, list* sTETPInshlist, list* newsegshlist,
...
///////////////////////////////////////////////////////////////////////////////
  ...
  // Pointer to the 'tetrahedron' that occuTETPIes all of "outer space".
  ...
  // The maximum btree depth (for bookkeeTETPIng).
  int max_btree_depth; 
...
///////////////////////////////////////////////////////////////////////////////
  ...
  tetgenmesh()
  {
    ...
    randomseed = 1l;
    macheps = 0.0;
    minfaceang = minfacetdihed = TETPI;
    b_steinerflag = false;
...
// sTETPIvot() finds the other subface (from this subface) that shares the
//   same edge.

inline void tetgenmesh::sTETPIvot(face& s1, face& s2) {
  shellface sptr = s1.sh[Orient(s1.shver)];
  sdecode(sptr, s2);
}

inline void tetgenmesh::sTETPIvotself(face& s) {
...
// tsTETPIvot() finds a subface abutting on this tetrahdera.

inline void tetgenmesh::tsTETPIvot(triface& t, face& s) {
  if ((t).tet[9] != NULL) {
...
// stTETPIvot() finds a tetrahedron abutting a subface.

inline void tetgenmesh::stTETPIvot(face& s, triface& t) {
...
// ssTETPIvot() finds a subsegment abutting a subface.

inline void tetgenmesh::ssTETPIvot(face& s, face& edge) {
...
inline void tetgenmesh::tssTETPIvot1(triface& t, face& s)
{
  if ((t).tet[8] != NULL) {
...
// The last significant bit of this integer is used by TETPInfect/puninfect.
...
// TETPInfect(), puninfect(), TETPInfected() -- primitives to flag or unflag
//   a point. The last bit of the integer '[pointindex+1]' is flaged.

inline void tetgenmesh::TETPInfect(point pt) {
...
inline bool tetgenmesh::TETPInfected(point pt) {
