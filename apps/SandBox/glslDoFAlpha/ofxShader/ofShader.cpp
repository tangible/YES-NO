...
//	if (GLEE_ARB_shader_objects){
		...
		glShaderSource(vertexShader, 1, (const char**)&vs, &length);
		length = strlen(fs);
		glShaderSource(fragmentShader, 1, (const char**)&fs, &length);
...
void ofShader::setUniform (char * name, float v1) {
	if(bLoaded ){
		glUniform1f(glGetUniformLocation(shader, name), v1);
	}
}

void ofShader::setUniform (char * name, float v1, float v2){
	if (bLoaded ){
		glUniform2f(glGetUniformLocation(shader, name), v1, v2);
	}
}
...
void ofShader::setUniform (char * name, float v1, float v2, float v3, float v4){
	if (bLoaded ){
		glUniform4f(glGetUniformLocation(shader, name), v1, v2, v3, v4);
	}
}
...
void ofShader::setUniform (char * name, int v1){
	if (bLoaded ){
		glUniform1i(glGetUniformLocation(shader, name), v1);
	}
}

void ofShader::setUniform (char * name, int v1, int v2){
	if (bLoaded ){
		glUniform2i(glGetUniformLocation(shader, name), v1, v2);
	}
}

void ofShader::setUniform (char * name, int v1, int v2, int v3){
	if (bLoaded ){
		glUniform3i(glGetUniformLocation(shader, name), v1, v2, v3);
	}
}

void ofShader::setUniform (char * name, int v1, int v2, int v3, int v4){
	if (bLoaded ){
		glUniform4i(glGetUniformLocation(shader, name), v1, v2, v3, v4);
	}
}
