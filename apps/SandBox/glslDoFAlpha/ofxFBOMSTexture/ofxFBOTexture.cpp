...
void ofxFBOTexture::initFBOs()
{
	...
	glRenderbufferStorageMultisampleEXT(GL_RENDERBUFFER_EXT, 4, GL_DEPTH_COMPONENT24, texData.tex_w, texData.tex_h);
	...
	glTexImage2D(texData.textureTarget, 0, texData.glType, texData.tex_w, texData.tex_h, 0, texData.glType, GL_FLOAT, 0);
...
void ofxFBOTexture::checkStatus()
{
	GLenum status = glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT);
	if(status != GL_FRAMEBUFFER_COMPLETE_EXT) {
		cout << "FBO failed to initialize.";
		std::string error;
		switch (status) {
