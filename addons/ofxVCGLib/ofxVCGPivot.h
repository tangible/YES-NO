...
#include "vcglib/vcg/space/index/grid_static_ptr.h"
#include "vcglib/vcg/complex/trimesh/closest.h"
...
namespace vcg {
	namespace tri {
		...
		public:
			...
			ofxVCGPivot(MESH &_mesh, float _radius, float _mindist = 0.05, float _crease = -0.5): 
			...
			bool pivot(Hinge &Hinge, int &candidate, Point3f &end_pivot, vector<int> &targets) {
		...
		private:
			...
			void killHinge(list<Hinge>::iterator e) {
			...
			void moveFront(list<Hinge>::iterator e) {
			...
			list<Hinge>::iterator touches(int v, list<Hinge>::iterator e) {
...
//			
//			ofxVCGPivot(MESH &_mesh, float _radius, float _mindist = 0.05, float _crease = -0.5): 
//			m(_mesh), radius(_radius), mindist(_mindist), crease(_crease) {
//				
//				
//				//Compute bounding box. (this may be passed as a parameter?
