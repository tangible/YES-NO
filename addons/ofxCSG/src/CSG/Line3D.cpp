...
//double Line3D::TOL = 1e-10f;
double Line3D::TOL = 1e-5f;
...
Line3D::Line3D(Face * face1, Face * face2)
{
...
Line3D::Line3D(const mlVector3D & directioni, const mlVector3D & pointi)
{
...
mlVector3D Line3D::getPoint()
{
	return point;
}

mlVector3D Line3D::getDirection()
{
	return direction;
}

void Line3D::setPoint(const mlVector3D & pointi)
{
	point = pointi;
}

void Line3D::setDirection(const mlVector3D & directioni)
{
	direction = directioni;
}

double Line3D::computePointToPointDistance(const mlVector3D & otherPoint)
{
...
mlVector3D Line3D::computeLineIntersection(Line3D * otherLine)
{
	//x = x1 + a1*t = x2 + b1*s
	//y = y1 + a2*t = y2 + b2*s
	//z = z1 + a3*t = z2 + b3*s
...
mlVector3D Line3D::computePlaneIntersection(const mlVector3D & normal, const mlVector3D & planePoint, bool & bResult)
{
	bResult = true;
